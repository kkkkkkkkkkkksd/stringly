...
## На будущее (бэклог, см. 02-features / 10-roadmap)

- **Статусы перевода (F-29).** Поле `Translation.status` уже есть. В будущем уточняем
  семантику под workflow в стиле Xcode String Catalog: `new` (ключ есть, перевода нет —
  внимание переводчика), `needs review` (изменился исходник/ключ → перевод устарел),
  `translated` (актуально). `needs review` ставится автоматически при изменении базового
  значения. На Шагах 3–4 в UI сознательно показываем только «пусто vs заполнено».
- **Версионирование / релизные каналы (F-30).** Вводим срез переводов по каналу
  (`planned | testing | released`) и/или версии-тегу. Минимально: связь
  `Translation`/набор ключей ↔ `channel`/`versionTag`; публичный API выбирает канал.
  Это отдельный механизм от аудита изменений (F-17, «кто/когда»).
