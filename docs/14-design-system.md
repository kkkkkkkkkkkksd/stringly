...
> **Исключение — маркетинговые «входные» экраны (лендинг, 404).** Это витрина, а не рабочий
> инструмент, поэтому там РАЗРЕШены градиенты, тёмный hero и более яркая палитра (направление
> «Gradient SaaS», см. `docs/13`). Бренд-цвет остаётся индиго (`#4F46E5`/`#7C3AED`), акценты —
> `#EC4899`/`#F59E0B`/`#0EA5E9`. **Рабочая область приложения (таблица, настройки) — строго
> плоская по правилам выше; яркий стиль за её пределы не выходит.**
...
**TopNav + AccountMenu** (рабочая область) — горизонтальная навигация сверху (высота 56px,
bg `--bg-surface`, border-bottom) **вместо левого сайдбара**: бренд слева (→ таблица),
контекст проекта, поиск-заглушка, справа — меню аккаунта/настроек. Левый сайдбар не
используем — вся ширина уходит под данные (важно при многих языках/длинном тексте).

**Menu / MenuItem** (`shared/ui/Menu`) — переиспользуемое выпадающее меню/поповер: панель
bg `--bg-surface`, border `--border-strong`, радиус lg, `--shadow-pop`; закрытие по Esc и
клику вне. Пункт: высота ~36px, hover bg `--bg-hover`, активный — текст `--primary-hover`,
правый слот для хинтов/бейджей/галочки. Меню аккаунта собрано из него.

**ProgressBar** (`shared/ui/ProgressBar`) — тонкий индикатор (высота 6px, радиус full, трек
`--bg-subtle`, заливка `--success` или `--primary`). Рейл прогресса по языкам и т.п.
